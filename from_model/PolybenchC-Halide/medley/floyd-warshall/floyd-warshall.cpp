...
static void init_array(int n, Buffer<num_t, 2> path) {
    ...
    // Note: Buffer layout is (x=j, y=i)
    init_path(j, i) = select(cond,
                             cast<num_t>(999),
                             base);

    // ------------------------
    // Schedule for init_path:
    // ------------------------
    //
    // - j is the fast-varying (x) dimension in the Buffer, so we
    //   reorder to make j innermost.
    // - Vectorize over j to get SIMD initialization.
    // - Parallelize over i (rows) for multi-core initialization.
    Target target = get_jit_target_from_environment();
    const int vec_width = target.natural_vector_size(type_of<num_t>());

    init_path
        .reorder(j, i)          // row-major order: j inner, i outer
        .vectorize(j, vec_width)
        .parallel(i);

    // Compile once, then realize into the provided buffer.
    init_path.compile_jit(target);
...
int main(int argc, char *argv[]) {
    ...
    //
    // Layout: Buffer(x=j, y=i) so that C path[i][j] is path(j, i).
    ...
    // Mapping between C and Halide coordinates:
    //   C: path[i][j] -> path_param(j, i)
    //   C: path[i][k] -> path_param(k, i)
    //   C: path[k][j] -> path_param(j, k)
    //
    // For a fixed k (k_param), and each (i, j) we compute:
    //   current = path[i][j]
    //   via_k   = path[i][k] + path[k][j]
    //   result  = min(current, via_k)
    //
    // We explicitly hoist path[i][k] into a scalar Expr 'a_ik' so that
    // Halide/LLVM can keep it in a register across the inner j-loop.
    Expr current = path_param(j, i);
    Expr a_ik    = path_param(k_param, i);        // path[i][k], independent of j
    Expr via_k   = a_ik + path_param(j, k_param); // path[i][k] + path[k][j]

    floyd_step(j, i) = Halide::min(current, via_k);

    // -----------------------------
    // Schedule for floyd_step:
    // -----------------------------
    //
    // For each k (managed on the host), we realize floyd_step over the
    // entire N×N matrix:
    //
    //   for i in [0, N):
    //     for j in [0, N):
    //       ...
    //
    // All reads come from path_param (current_path), all writes go into
    // next_path, so for fixed k all (i,j) updates are independent.
    // That allows:
    //   - tiling in (j,i) for cache locality,
    //   - parallelization across tiles,
    //   - vectorization across j (unit-stride dimension).
    Target target = get_jit_target_from_environment();
    const int vec_width = target.natural_vector_size(type_of<num_t>());

    // Tunable tile sizes (chosen for typical cache sizes on modern CPUs).
    const int tile_j = 64;  // tile width in columns (j, fast dimension)
    const int tile_i = 32;  // tile height in rows (i, slow dimension)

    Var jo("jo"), ji("ji");  // outer and inner j
    Var io("io"), ii("ii");  // outer and inner i

    floyd_step
        // Optional explicit bounds hint (matches buffer extent).
        .bound(j, 0, n)
        .bound(i, 0, n)
        // Tile the (j, i) space into tile_j × tile_i blocks.
        .tile(j, i, jo, io, ji, ii, tile_j, tile_i)
        // Within each tile, iterate j fastest for unit-stride SIMD.
        .reorder(ji, ii, jo, io)
        // Vectorize along ji (inner j within a tile).
        .vectorize(ji, vec_width)
        // Parallelize across row-tiles (io) – coarse-grained tasks.
        .parallel(io);
    ...
    // Outer DP loop over k is kept on the host and strictly sequential
    // (Floyd–Warshall requires that k advance in order).
    for (int k = 0; k < n; k++) {
        // Bind the current path and the iteration index k
        path_param.set(*current_path);
        k_param.set(k);

        // Compute the next path matrix for this k.
        //
        // Thanks to the schedule above, this realization:
        //   - tiles (j,i) into cache-friendly blocks,
        //   - executes tiles in parallel across CPU cores,
        //   - vectorizes across j inside each tile.
        floyd_step.realize(*next_path);

        // Swap buffers for the next iteration (O(1) pointer swap, no copy).
