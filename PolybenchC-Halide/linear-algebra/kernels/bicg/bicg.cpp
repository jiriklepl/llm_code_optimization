...
int main(int argc, char *argv[]) {
    ...
    // Simple schedules that respect the 1D nature of s and q.
    // The pure definitions are already per-element; we just lightly
    // reorder the update loops to mirror (outer index, reduction index).
    s_func.compute_root();
    s_func.update().reorder(ri, j);

    q_func.compute_root();
    q_func.update().reorder(rj, i);
