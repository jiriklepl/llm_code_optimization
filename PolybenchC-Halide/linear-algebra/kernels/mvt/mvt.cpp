...
int main(int argc, char *argv[]) {
    ...
    // Optional: make the loop order explicit for the update steps;
    // default is outer over pure vars (i), inner over reduction vars (r),
    // which matches the C code's i-outer, j-inner structure.
    mvt_x1.update().reorder(i, r);
    mvt_x2.update().reorder(i, r);

    // JIT-compile the two Funcs
    mvt_x1.compile_jit();
    mvt_x2.compile_jit();
