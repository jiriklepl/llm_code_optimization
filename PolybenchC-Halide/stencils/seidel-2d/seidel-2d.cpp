...
int main(int argc, char *argv[]) {
    ...
    Expr denom = cast<num_t>(SCALAR_VAL(9.0));
    ...
    // Default schedule is fine: outer loop over y (rows), inner over x (cols),
    // which matches the original C traversal order i (rows), then j (cols).

    // JIT-compile the per-time-step pipeline once
    seidel_step.compile_jit();
