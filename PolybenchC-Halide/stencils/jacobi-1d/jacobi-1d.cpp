...
int main(int argc, char *argv[]) {
    ...
    // Simple schedules: compute each stage at root; vectorize the inner loop.
    jacobi_A_to_B.compute_root().vectorize(k, 8);
    jacobi_B_to_A.compute_root().vectorize(k, 8);
    ...
    // JIT-compile the per-sweep kernels once.
    jacobi_A_to_B.compile_jit();
    jacobi_B_to_A.compile_jit();
