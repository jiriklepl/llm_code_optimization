...
for framework in "${frameworks[@]}"; do
    ...
    while : ; do
        ((current_part++))
        file_path=$(printf "requests/%s/from_model/%s_full_part%02d.jsonl" "$provider" "$framework" "$current_part")
...
options=$(for path in "${paths[@]}"; do
    framework=$(echo "$path" | grep -oE '/from_model/([a-z]+)_' | cut -d'/' -f3 | cut -d'_' -f1)
...
for option in $options; do
    ...
    from_model "$framework" "$provider" "full_part$part"
	sleep "$sleep_interval"

	pending=$(receive "$provider" 2>&1 | grep -oE "([0-9]+|No) pending" | cut -d' ' -f1)
	if ! [ "$pending" = "No" ]; then
        echo "Pending from_model requests: $pending"
    fi
done
