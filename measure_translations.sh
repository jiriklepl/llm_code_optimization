...
for DATASET_SIZE in "${DATASET_SIZEs[@]}"; do
	...
	if [ "${3:-}" != "noc" ]; then
		for algorithm in ${algorithms}; do
			...
			if grep -q "^c,standard,${algorithm},1,.*,${DATASET_SIZE}$" "${times_file}"; then
				actual_runs=$(( runs - $(grep -c "^c,standard,${algorithm},.*,.*,${DATASET_SIZE}$" "${times_file}") ))
			fi

			if [[ ${actual_runs} -le 0 ]]; then
				echo "Skipping C standard ${algorithm} for dataset size ${DATASET_SIZE} (already done)." | tee -a "${output_file}"
				continue
			fi

			run "${algorithm}" "c" "translation" "${actual_runs}" | awk '{print  $0 ",1,'"${DATASET_SIZE}"'"}' | tee -a "${times_file}"
	...
	for framework in "${FRAMEWORKS_TO_RUN[@]}"; do
		for rep in $(seq "${reps}"); do
			...
			for algorithm in ${algorithms}; do
				actual_runs=${runs}
				if grep -q "^${framework},${algorithm},.*,${rep},${DATASET_SIZE}$" "${validation_file}"; then
					validation=$(grep "^${framework},${algorithm},.*,${rep},${DATASET_SIZE}$" "${validation_file}")
					actual_runs=$(( runs - $(grep -c "^${framework},standard,${algorithm},.*,${rep},${DATASET_SIZE}$" "${times_file}") ))
				else
					validation=$(check_translation "${algorithm}" "${framework}" "${abs_tolerance}" "${rel_tolerance}" | awk '{print $0 ",'"${rep}"','"${DATASET_SIZE}"'"}' | tee -a "${validation_file}")
					echo "${validation}"
				fi

				if [[ ${actual_runs} -le 0 ]]; then
					echo "Skipping ${framework} translation ${algorithm} repetition ${rep} for dataset size ${DATASET_SIZE} (already done)." | tee -a "${output_file}"
					continue
				fi
