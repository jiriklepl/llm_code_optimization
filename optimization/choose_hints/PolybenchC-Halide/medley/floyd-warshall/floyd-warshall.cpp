...
static void init_array(int n, Buffer<num_t, 2> path) {
    ...
    // Buffer layout is (x=j, y=i) to match path[i][j] -> path(j, i)
    ...
    //
    // We iterate row-major (j is fast), vectorize across j, and
    // parallelize across i. This preserves semantics but gives
    // good CPU utilization and memory bandwidth during initialization.
    const int vec_width_init = 8;  // 8-wide vectors are a good fit for AVX2 on x86
    init_path
        .compute_root()
        .reorder(j, i)          // j = x is innermost, contiguous in memory
        .vectorize(j, vec_width_init)
        .parallel(i);
...
int main(int argc, char *argv[]) {
    ...
    // ---- Schedule for a single Floyd–Warshall step ----
    //
    // Each (i,j) for a fixed k is independent, so we can:
    //   - iterate row-major (j inner, contiguous in memory),
    //   - vectorize across j (x dimension),
    //   - parallelize across i (y dimension).
    //
    // This improves data locality (streaming access along j), uses SIMD
    // on each row, and distributes rows across cores.
    const int vec_width_step = 8;  // 8-wide vectors (e.g. 8x32-bit values)
    floyd_step
        .compute_root()
        .reorder(j, i)             // j (x) is innermost, matches layout
        .vectorize(j, vec_width_step)
        .parallel(i);
