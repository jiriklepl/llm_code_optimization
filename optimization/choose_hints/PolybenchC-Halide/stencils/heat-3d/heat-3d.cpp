 ...
 *
 * This is expressed as a Halide Func and scheduled to use
 * CPU parallelism and vectorization.
 */
static void init_array(int n,
    ...
    Expr val = cast<num_t>(i + j + (n - k)) *
               cast<num_t>(10) / cast<num_t>(n);
    init(k, j, i) = val;

    // Simple CPU schedule: vectorize along the fastest-varying
    // dimension (k) and parallelize across outer i-scanlines.
    Target target = get_jit_target_from_environment();
    const int vec_width =
        target.natural_vector_size(type_of<num_t>());

    Var ko("ko"), ki("ki");
    init
        .compute_root()
        .split(k, ko, ki, vec_width)
        // Var order after split: [ko, ki, j, i]
        // Reorder to [ki, ko, j, i] so that:
        //   loops: for i (outer, parallel), for j, for ko, for ki (innermost, vectorized)
        .reorder(ki, ko, j, i)
        .vectorize(ki)
        .parallel(i);
...
int main(int argc, char *argv[]) {
    ...
    // ---------------- Scheduling for the stencil kernels ----------------
    //
    // We tune for a modern x86-64 CPU:
    //   - vectorize along k (dim 0, contiguous in memory)
    //   - parallelize across the outer i-dimension
    //   - leave j as a serial inner loop for good cache reuse
    //
    // This schedule is applied identically to B_from_A and A_from_B.
    Target target = get_jit_target_from_environment();
    const int vec_width =
        target.natural_vector_size(type_of<num_t>());

    auto schedule_stencil = [&](Func &f) {
        Var ko("ko"), ki("ki");

        f
            .compute_root()
            // Split k into an outer and an inner (vectorized) part.
            .split(k, ko, ki, vec_width)
            // Current var order after split: [ko, ki, j, i]
            // Reorder so that:
            //   loops: for i (outer, parallel),
            //          for j,
            //          for ko (coarse k blocks),
            //          for ki (innermost, contiguous, vectorized)
            .reorder(ki, ko, j, i)
            .vectorize(ki)
            .parallel(i);
    };

    schedule_stencil(B_from_A);
    schedule_stencil(A_from_B);
