...
#include "medley/floyd-warshall/floyd-warshall.h"
...
# Extern to express data-dependent min without Exo conditionals
class _Min2(Extern):
    def __init__(self):
        super().__init__("min2")
    ...
    def typecheck(self, args):
        ...
        # Accept any numeric/scalar type; return the type of the first argument
        return args[0].type

    def compile(self, args, prim_type):
        # Emit a C ternary min between the two arguments
        return f"(({prim_type}){args[0]} < ({prim_type}){args[1]} ? ({prim_type}){args[0]} : ({prim_type}){args[1]})"
...
int main(int argc, char** argv)
{
  ...
  kernel_floyd_warshall(/*ctxt=*/NULL, n,
