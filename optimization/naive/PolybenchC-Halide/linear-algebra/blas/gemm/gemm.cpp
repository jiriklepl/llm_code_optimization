...
static void init_array(int ni, int nj, int nk,
    ...
    *beta  = static_cast<num_t>(1.2);

    Var i("i"), j("j"), k("k");
    Func init_A("init_A"), init_B("init_B"), init_C("init_C");

    // Use the host target to pick a reasonable vector width.
    ...
    // Buffer layout: C(j, i)
    init_C(j, i) =
        cast<num_t>(((i * j + 1) % ni)) / cast<num_t>(ni);

    // A[i][j] = (i*(j+1) % nk) / nk;
    // Buffer layout: A(k, i) where k is the "j" from the C code
    init_A(k, i) =
        cast<num_t>((i * (k + 1) % nk)) / cast<num_t>(nk);

    // B[i][j] = (i*(j+2) % nj) / nj;
    // Buffer layout: B(j, k) where i in C code is our k, and j is j
    init_B(j, k) =
        cast<num_t>((k * (j + 2) % nj)) / cast<num_t>(nj);

    // Simple but effective schedules for the initialization:
    //  - parallelize across the slow dimension (rows)
    //  - vectorize across the fast dimension (columns)
    init_C
        .compute_root()
        .parallel(i)
...
int main(int argc, char *argv[]) {
    ...
    //
    // Using Expr(beta) ensures that beta is lifted into the Halide expression
    // graph as a scalar. The same applies for alpha in the update definition.
    ...
    //
    // Goals:
    //   - Make j (columns) the innermost loop for unit-stride memory access.
    //   - Tile i and j to improve cache locality.
    //   - Split the reduction domain k to improve cache use over A and B.
    //   - Vectorize across j.
    //   - Parallelize across tiles of i (rows) for multi-core CPUs.
    //
    Target target = get_host_target();
    const int vec_width = target.natural_vector_size(type_of<num_t>());

    // Tile sizes; chosen conservatively to work well on typical desktop CPUs.
    const int tile_i = 64;  // tile size along i (rows)
    const int tile_j = 64;  // tile size along j (columns)
    const int k_block = 32; // reduction blocking factor along k

    // Tiled and parallelized loops.
    Var io("io"), jo("jo"), ii("ii"), ji("ji");
    RVar ko("ko"), ki("ki");

    // Compute gemm at the root and then tile (j, i) into (jo, io, ji, ii).
    gemm.compute_root();

    // Tile the output domain:
    //   j -> (jo, ji)
    //   i -> (io, ii)
    // Layout after reorder: ji (innermost, vectorized), then ii, then jo, then io.
    gemm
    ...
    //   - Split k into (ko, ki) to improve locality and enable better
    //     instruction scheduling in the inner loops.
    //   - Keep ji innermost (unit stride), then ki, then ii, then ko, jo, io.
    gemm.update()
        .split(k, ko, ki, k_block)
    ...
    // Compile the kernel to machine code (JIT) for the detected host target
    gemm.compile_jit(target);

    // Measure execution time of the GEMM kernel
    auto start = std::chrono::high_resolution_clock::now();

    // Run kernel: write the result into C (in-place update of C as per original C code)
    gemm.realize(C);
