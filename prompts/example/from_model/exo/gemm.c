...
def kernel_gemm(
    ...
    B: DATA_TYPE[nj, nk] @ DRAM,
):
    # Rewrite manually as the scheduling rejects reordering loops with multiple nested loops
    for J in seq(0, (nj + TILE - 1) / TILE):
        for I in seq(0, (ni + TILE - 1) / TILE):
            for i in seq(0, TILE):
                if TILE * I + i < ni:
                    for j in seq(0, TILE):
                        if TILE * J + j < nj:
                            C[TILE * I + i, TILE * J + j] = beta * C[TILE * I + i, TILE * J + j]
            for K in seq(0, (nk + TILE - 1) / TILE):
                for i in seq(0, TILE):
                    if TILE * I + i < ni:
                        for k in seq(0, TILE):
                            if TILE * K + k < nk:
                                for j in seq(0, TILE):
                                    if TILE * J + j < nj:
                                        C[TILE * I + i, TILE * J + j] += alpha * A[TILE * I + i, TILE * K + k] * B[TILE * J + j, TILE * K + k]
# schedule would be preferred but cannot handle the nested loops
EXO END
*/
